...
class EmbeddingRepo:
    ...
    async def search_similar(
        ...
        query = select(
            CodeEmbedding,
            func.cosine_distance(CodeEmbedding.embedding, query_embedding).label("distance")
        )
        ...
        query = query.where(
            func.cosine_distance(CodeEmbedding.embedding, query_embedding) <= (1 - threshold)
        ...
        return [
            ...
            for embedding, distance in result.all()
        ]
...
class CommitRepo:
    ...
    async def get_by_sha(self, repo_id: int, sha: str) -> Optional[Commit]:
        """Get commit by SHA."""
        result = await self.session.execute(
            select(Commit).where(
                and_(
    ...
    async def get_latest(self, repo_id: int) -> Optional[Commit]:
