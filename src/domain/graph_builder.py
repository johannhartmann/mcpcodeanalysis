...
{{
  ...
  "ubiquitous_language": {{"term": "definition"}}
        ...
        try:
            ...
            name = aggregate_roots[0]["name"] if aggregate_roots else entities[0]["name"]
    ...
    async def analyze_context_relationships(
        ...
        for u, v, data in graph.edges(data=True):
            ...
            if u_context is not None and v_context is not None and u_context != v_context:
    ...
    def _determine_context_relationship_type(
        ...
        elif "orchestrates" in rel_types:
            return "customer_supplier"
        elif edge_data["count"] > 5:
            return "shared_kernel"
        elif "validates" in rel_types:
