...
from src.database.models import CodeEmbedding, File, Function, Class, Module
...
class DomainAwareSearch:
    ...
    async def _find_relevant_entities(
        ...
        if scope != DomainSearchScope.ALL:
            ...
            if scope in entity_types:
                query = query.where(
                    DomainEntity.entity_type.in_(entity_types[scope])
                )
        ...
        if concepts:
            ...
            for concept in concepts:
                concept_conditions.append(
                    DomainEntity.name.ilike(f"%{concept}%")
                )
    ...
    async def _domain_weighted_search(
        ...
        query = query.add_columns(
            (
                text("base_similarity * domain_boost")
            ).label("final_score")
        )
            ...
            if domain_entities:
                ...
                if domain_entities:
                    membership_result = await self.db_session.execute(
                        ...
                        .where(
                            BoundedContextMembership.domain_entity_id == domain_entities[0].id
                        )
