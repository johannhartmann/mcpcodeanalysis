...
class MCPCodeAnalysisServer:
    ...
    def __init__(self):
        ...
        self.code_search_tools: Optional[CodeSearchTools] = None
        self.code_analysis_tools: Optional[CodeAnalysisTools] = None
        self.repo_management_tools: Optional[RepositoryManagementTools] = None
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Log configuration
            logger.info(f"Server initialized with {len(self.settings.repositories)} repositories")
    ...
    async def _register_tools(self):
        ...
        async with self.session_factory() as session:
            ...
            self.code_search_tools = CodeSearchTools(session, self.openai_client, self.mcp)
    ...
    async def scan_repository(
        ...
        async with self.get_session() as session:
            ...
            if generate_embeddings and self.openai_client and result.get("repository_id"):
                embedding_service = EmbeddingService(session, self.openai_client)
                embeddings_result = await embedding_service.create_repository_embeddings(
            ...
            if repository_id:
                ...
                for repo in repositories:
                    try:
                        ...
                        logger.error(f"Failed to update embeddings for repository {repo['id']}: {e}")
                        results.append({
                            "repository_id": repo["id"],
                            "repository_name": repo["name"],
                            "error": str(e),
