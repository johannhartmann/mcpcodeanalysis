...
class FindTool:
    ...
    async def find_definition(
        ...
        try:
            async with get_session() as session:
                async with get_repositories(session) as repos:
                    ...
                    for result in results:
                        ...
                        file = await repos["file"].get_by_id(module.file_id) if module else None
                        ...
                        definition = {
                            ...
                            "location": {
                                ...
                                "line": entity.start_line if hasattr(entity, "start_line") else None,
                            },
            ...
            logger.error(f"Error in find_definition: {e}")
            return [{
                "error": str(e),
                "name": name,
                "type": type
        ...
        try:
            async with get_session() as session:
                async with get_repositories(session) as repos:
                    ...
                    definitions = await repos["code_entity"].find_by_name(function_or_class)
            ...
            logger.error(f"Error in find_usage: {e}")
            return [{
                "error": str(e),
