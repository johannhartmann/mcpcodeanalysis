...
class AddRepositoryRequest(BaseModel):
    ...
    branch: Optional[str] = Field(None, description="Branch to track")
...
class RepositoryManagementTools:
    ...
    def __init__(
        ...
        self.embedding_service = (
            EmbeddingService(db_session, openai_client) 
    ...
    async def register_tools(self):
            ...
            try:
                ...
                if request.scan_immediately:
                    ...
                    if request.generate_embeddings and self.embedding_service:
                        embedding_result = await self.embedding_service.create_repository_embeddings(
        ...
        async def scan_repository(request: ScanRepositoryRequest) -> Dict[str, Any]:
            ...
            try:
                ...
                repo_config = RepositoryConfig(
                    ...
                    access_token=SecretStr(repo.access_token_id) if repo.access_token_id else None,
                )
        ...
        async def get_repository_stats(
            ...
            try:
                ...
                stats["languages"] = {
                    row[0]: row[1] for row in lang_result if row[0]
                }
                
                # Get embedding statistics if available
                if self.embedding_service:
                    vector_search = VectorSearch(self.db_session, self.openai_client)
                    embedding_stats = await vector_search.get_repository_stats(request.repository_id)
                    ...
                    stats["recent_commits"] = [
                        {
                            ...
                            "files_changed": len(c.files_changed) if c.files_changed else 0,
                        }
                        for c in commits
                    ]
