"""PHP code parser using TreeSitter."""

from pathlib import Path
from typing import Any

from src.logger import get_logger
from src.parser.treesitter_parser import PHPParser as TreeSitterPHPParser
...
class PHPCodeParser:
    ...
    def parse_file(self, file_path: Path) -> dict[str, Any]:
        """Parse a PHP file and extract all code entities."""
        try:
            # Read file content
            ...
            logger.exception("Error parsing PHP file %s", file_path)
            msg = f"Failed to parse PHP file: {file_path}"
            raise ParserError(
                msg,
                str(file_path),
            ) from e
