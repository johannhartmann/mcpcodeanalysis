...
class TreeSitterParser:
    ...
    def parse_content(self, content: bytes) -> Optional[tree_sitter.Tree]:
    ...
    def get_docstring(self, node: tree_sitter.Node, content: bytes) -> Optional[str]:
...
class PythonParser(TreeSitterParser):
    ...
    def extract_imports(self, tree: tree_sitter.Tree, content: bytes) -> List[Dict[str, Any]]:
        ...
        for node in import_nodes:
            ...
            for child in node.children:
                if child.type == "dotted_name" or child.type == "identifier":
                    import_data["imported_names"].append(self.get_node_text(child, content))
            
            imports.append(import_data)
        
        # Find from imports
        from_import_nodes = self.find_nodes_by_type(tree.root_node, "import_from_statement")
        for node in from_import_nodes:
            ...
            for child in node.children:
                ...
                elif child.type == "import_list" or child.type == "identifier":
                    if child.type == "identifier":
                        import_data["imported_names"].append(self.get_node_text(child, content))
                    else:
                        # Handle import list
                        for name_node in child.children:
                            if name_node.type == "identifier" or name_node.type == "dotted_name":
                                import_data["imported_names"].append(self.get_node_text(name_node, content))
    ...
    def extract_functions(
        ...
        content: bytes,
        parent_class: Optional[tree_sitter.Node] = None
    ) -> List[Dict[str, Any]]:
        """Extract function definitions."""
        functions = []
        root = parent_class if parent_class else tree.root_node
        
        function_nodes = self.find_nodes_by_type(root, "function_definition", max_depth=2 if parent_class else None)
        ...
        for node in function_nodes:
            ...
            for child in node.children:
                if child.type == "identifier":
                    ...
                    func_data["return_type"] = self.get_node_text(child, content).strip("->").strip()
                elif child.type == "block":
                    # Check if it's a generator
                    if self.find_nodes_by_type(child, "yield_statement") or self.find_nodes_by_type(child, "yield_expression"):
    ...
    def extract_classes(self, tree: tree_sitter.Tree, content: bytes) -> List[Dict[str, Any]]:
                ...
                if child.type == "identifier":
                    ...
                    for arg in child.children:
                        if arg.type == "identifier" or arg.type == "attribute":
                            class_data["base_classes"].append(self.get_node_text(arg, content))
    ...
    def extract_module_info(self, tree: tree_sitter.Tree, content: bytes) -> Dict[str, Any]:
    ...
    def _extract_parameters(self, params_node: tree_sitter.Node, content: bytes) -> List[Dict[str, Any]]:
                ...
                if child.type == "identifier":
                    ...
                    for subchild in child.children:
                        if subchild.type == "identifier":
                            ...
                            param_data["type"] = self.get_node_text(subchild, content).strip(":").strip()
                        elif subchild.type not in (":", "="):
                            # Default value
                            param_data["default"] = self.get_node_text(subchild, content)
                
                if param_data["name"] and param_data["name"] not in ("self", "cls", "*", "**", "/"):
                    parameters.append(param_data)
        
        return parameters
    
    def _get_module_docstring(self, tree: tree_sitter.Tree, content: bytes) -> Optional[str]:
