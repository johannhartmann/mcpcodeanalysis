...
class CodeAggregator:
    ...
    def _interpret_function(
        ...
        parts.append(f"This is {'an' if func.is_async else 'a'} {func_type} named '{func.name}'")
        ...
        # Parameters
        if func.parameters:
            param_names = [p["name"] for p in func.parameters if p["name"] not in ("self", "cls")]
            if param_names:
                parts.append(f"It takes {len(param_names)} parameter(s): {', '.join(param_names)}")
        ...
        if func.is_property:
            ...
            parts.append("This is a class method that receives the class as first argument")
        
        return ". ".join(parts) + "."
    
    def _interpret_class(
        self,
        cls: Class,
