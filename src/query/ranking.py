...
class ResultRanker:
    ...
    def _calculate_popularity_score(self, result: Dict[str, Any]) -> float:
        ...
        common_names = {"init", "main", "get", "set", "create", "update", "delete", "process", "handle"}
    ...
    def format_results(
        ...
        for result in results:
            ...
            formatted_result = {
                ...
                "match_type": "interpreted" if result["similarity"]["interpreted"] > result["similarity"]["raw"] else "raw",
            }
            ...
            if include_context:
                ...
                if entity_data.get("type") == "function":
                    formatted_result["context"]["parameters"] = entity_data.get("parameters", [])
                    formatted_result["context"]["return_type"] = entity_data.get("return_type")
                elif entity_data.get("type") == "class":
                    formatted_result["context"]["base_classes"] = entity_data.get("base_classes", [])
                    formatted_result["context"]["is_abstract"] = entity_data.get("is_abstract", False)
