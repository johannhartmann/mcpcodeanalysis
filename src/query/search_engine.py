...
class SearchEngine:
    ...
    async def _merge_and_rank_results(
        ...
        for (entity_type, entity_id), scores in entity_scores.items():
            # Load entity details
            entity_data = await self._load_entity_details(
                repos,
                entity_type,
                entity_id
            )
            ...
            if repository_filter and entity_data.get("repository_name") != repository_filter:
                continue
            ...
            result = {
                ...
                "matched_content": scores.get("interpreted_content") or scores.get("raw_content"),
            }
