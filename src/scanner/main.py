                ...
                async with get_repositories(session) as repos:
                    ...
                    if not db_repo:
                        db_repo = await repos["repository"].create(
                            ...
                            name=repo_info["name"],
                            default_branch=repo_config.get("branch") or repo_info["default_branch"],
                            access_token_id=repo_config.get("access_token"),
                        )
                    
                    # Clone or update repository
                    if not self.git_sync.get_repo_path(repo_info["owner"], repo_info["name"]).exists():
                        # Initial clone
                        ...
                        # Process all files
                        await self.process_all_files(db_repo, repo_info["owner"], repo_info["name"])
                    else:
                    ...
                    repo_path = self.git_sync.get_repo_path(repo_info["owner"], repo_info["name"])
            ...
            if entities:
                ...
                for class_data in entities["classes"]:
                    await repos["code_entity"].create_class(
                        module_id=1,  # TODO: Get actual module ID
                        **class_data
                    )
            ...
            async with get_session() as session:
                async with get_repositories(session) as repos:
                    if event_type == "deleted":
                        ...
                        db_file = await repos["file"].get_by_path(
                            repo_id,
                            file_path
                        )
                        if db_file:
