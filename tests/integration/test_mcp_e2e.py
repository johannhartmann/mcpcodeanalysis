...
from src.database.init_db import init_database, get_session_factory
from src.database.models import Repository, File, Module, Function, Class, CodeEmbedding
...
''')
    ...
    async def test_full_workflow(self, db_session, openai_client, sample_repo):
        ...
        assert "Calculator" in [c["name"] for c in analysis_result["analysis"]["class_list"]]
...
''')
        ...
        subprocess.run(["git", "commit", "-m", "Add helpers"], cwd=sample_repo, check=True)
    ...
    async def test_search_functionality(self, db_session, openai_client, sample_repo):
        ...
        calc_results = [r for r in keyword_results["results"] if r["entity"]["name"] == "Calculator"]
